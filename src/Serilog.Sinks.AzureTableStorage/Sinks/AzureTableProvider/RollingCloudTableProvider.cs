...
{
    public class RollingCloudTableProvider : ICloudTableProvider
    {
        private readonly int _waitTimeoutMilliseconds = Timeout.Infinite;
        private readonly CloudTableClient _cloudTableClient;
        private readonly string _storageTableNamePrefix;
        private readonly bool _bypassTableCreationValidation;
        private CloudTable _cloudTable;

        private static string Ticks => DateTime.Today.Ticks.ToString();

        public RollingCloudTableProvider(CloudStorageAccount storageAccount,
                                         string storageTableNamePrefix,
                                         bool bypassTableCreationValidation)
        {
            _cloudTableClient = storageAccount.CreateCloudTableClient();
            _storageTableNamePrefix = storageTableNamePrefix;
            _bypassTableCreationValidation = bypassTableCreationValidation;
        }

        public CloudTable GetCloudTable()
        {
            if (_cloudTable == null || _cloudTable.Name.Remove(0, _storageTableNamePrefix.Length) == Ticks)
            {
                var storageTableName = $"{_storageTableNamePrefix}{Ticks}";
                _cloudTable = _cloudTableClient.GetTableReference(storageTableName);
